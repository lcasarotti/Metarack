...
// GUI / accessibility path (Windows + macOS). L'interfaccia accessibile legge l'albero dei
...
#else
	#include <dlfcn.h> // dladdr() — percorso del modulo, vedi getModuleDir()
#endif
#if defined ARCH_MAC
	// Su macOS AccessibleWindow::create() prende direttamente la GLFWwindow* (risolve da sé
	// la NSWindow), quindi serve solo l'header GLFW normale: niente glfw3native.
	#include <GLFW/glfw3.h>
#endif
...
// troverebbero. Risolviamo prendendo il modulo che contiene questa funzione — su Windows
// con GetModuleHandleEx FROM_ADDRESS, su macOS/Linux con dladdr, che è la stessa idea:
// "quale immagine caricata contiene questo indirizzo di codice?".
std::string getModuleDir() {
	...
	// Nessun modulo identificabile (non dovrebbe accadere per una libreria caricata):
	// la cartella di lavoro è l'unico ripiego, ed è quella dell'host.
...
// modulo stesso (il .clap accanto a res/), ma un bundle VST3 mette il binario in
// Contents/x86_64-win/ (Windows) o Contents/MacOS/ (macOS), qualche livello sotto.
// Risaliamo finché non troviamo res/.
...
// Contents/Resources, un livello sopra la cartella del modulo (Contents/<arch>/ su Windows,
// Contents/MacOS/ su macOS — la risalita di un livello vale identica per entrambi). Se la
...
// ottenere un grafo pass-through verificabile: col percorso GUI il rack di default è vuoto
// (è l'utente a instradare i moduli dalla finestra accessibile), quindi senza questo
// l'uscita sarebbe silenziosa per costruzione. Mai impostata da un DAW: in produzione
// questo ramo non si attiva.
...
#else
	const char* v = getenv(TEST_PASSTHROUGH_ENV);
	return v && v[0];
#endif
}
...
#else
	// Senza layer accessibile (Linux) resta la modalità headless bare-engine.
...
#if defined ARCH_MAC
		// glfwInit() su macOS ha effetti collaterali PROCESS-WIDE che dentro una DAW non
		// sono accettabili: rackhost_mac.mm li neutralizza attorno alle due init.
		mac::initGuiSubsystemsHosted();
#else
		ui::init();
		window::init();
#endif
...
#else
	// macOS: create() prende la GLFWwindow* e risolve da sé la NSWindow. pluginMode è un
	// parametro (non un campo) perché decide cose fatte DENTRO create: niente menu bar di
	// NSApp — che appartiene alla DAW — e un timer proprio al posto del run loop di Rack.
	// Nasce nascosta: la mostra l'adapter.
	inst->accessibleWindow = accessible::AccessibleWindow::create(APP->window->win, true);
#endif
	glfwHideWindow(APP->window->win);
#else
	// --- Senza layer accessibile (Linux): percorso headless bare-engine ---------------
...
void destroyInstance(Instance* inst) {
	...
	// referenzia (instance singleton, timer). Il suo distruttore smonta la propria finestra
	// nativa e, su macOS, ferma il timer di drain.
...
// Su macOS non c'è nessuna guerra per il focus da combattere: il pannello accessibile è
// una finestra del PROCESSO DELLA DAW, quindi portarla in primo piano è una normale
// makeKeyAndOrderFront (dentro setVisible) e non la richiesta cross-process che su Windows
// obbliga ad AttachThreadInput. Per lo stesso motivo non serve un equivalente di
// guiSetTransient/F6: ⌘` (finestra successiva) passa nativamente tra il pannello e le
// finestre della DAW, perché per AppKit sono tutte finestre della stessa applicazione.

bool guiShow(Instance* inst) {
	if (!inst->accessibleWindow)
		return false;
	inst->accessibleWindow->setVisible(true);
	return true;
}
...
bool guiHide(Instance* inst) {
	if (!inst->accessibleWindow)
		return false;
	inst->accessibleWindow->setVisible(false);
	return true;
}

bool guiGetSize(Instance* inst, uint32_t* width, uint32_t* height) {
	// Il pannello si dimensiona da sé sulla finestra Rack (nascosta, 1024x720): questi
	// valori servono solo agli host che chiedono una dimensione prima di aprire l'editor.
	(void) inst;
	*width = 900;
	*height = 600;
	return true;
