...
// Percorso CON GUI: l'adapter costruisce l'albero dei widget e la finestra accessibile
// (Windows/NVDA, macOS/VoiceOver). Fuori di lì resta il percorso headless bare-engine:
// engine nudo, nessun widget, nessuna finestra. Un solo simbolo per non dover ripetere
// ovunque la coppia di piattaforme, e per rendere esplicito che il discrimine è
// "esiste un layer accessibile?", non "quale sistema operativo?".
#if defined ARCH_WIN || defined ARCH_MAC
	#define RACKHOST_GUI 1
#endif
...
#if defined ARCH_MAC
// Su macOS l'inizializzazione dei sottosistemi GUI va protetta: glfwInit() ha effetti
// PROCESS-WIDE che dentro una DAW sono inaccettabili (si prende il delegate di NSApp e
// cambia la working directory del processo). Implementata in rackhost_mac.mm, che salva e
// ripristina entrambe le cose attorno a ui::init()/window::init().
namespace mac {
