...
//
// Build: target Makefile `vst3test` (console, niente -mwindows). Esegui dalla root del repo.
//
// Su macOS il bundle non ha stub: il modulo È il binario in Contents/MacOS, caricato con
// dlopen, e gli entry point sono bundleEntry/bundleExit invece di InitDll/ExitDll. Il resto
// del ciclo di vita è identico. L'unica parte saltata è la GUI: su macOS create_view()
// restituisce ancora nullptr (nessun segnaposto NSView), e il test lo verifica.
...
#include <arch.hpp> // ARCH_WIN / ARCH_MAC
	...
	// L'harness è compilato come Objective-C++ su macOS (vedi VST3TEST_FLAGS nel Makefile):
	// per testare attached() serve una NSView vera, come quella che passa un host.
...
#else
	static const char* const kDefaultBundle = "MetaRack.vst3";
	static std::string g_bundleArg;
#endif

typedef bool (*InitDllFn)(void);
typedef bool (*ExitDllFn)(void);
typedef bool (*BundleEntryFn)(void*);
typedef bool (*BundleExitFn)(void);
...
static int run() {
	...
	// (Fuori da Windows non serve: il percorso headless di rackhost::createInstance cabla
	// già DAW in -> DAW out da sé, non essendoci una finestra da cui instradare a mano.)
...
#else
	// Col percorso GUI il rack di default è vuoto (è l'utente a instradare i moduli dalla
	// finestra accessibile), quindi senza questo l'uscita sarebbe silenziosa per costruzione
	// e il test del ponte audio non direbbe nulla. Va impostata PRIMA del dlopen: la legge
	// l'init dell'istanza.
	setenv("METARACK_TEST_PASSTHROUGH", "1", 1);

	const std::string bundle = g_bundleArg.empty() ? kDefaultBundle : g_bundleArg;
	const std::string modulePath = bundle + "/Contents/MacOS/MetaRack";
	std::printf("== RackVst3Test: carico %s ==\n", modulePath.c_str());

	// RTLD_LOCAL: nessun simbolo del plugin finisce nel namespace globale, come fa un host
	// reale. Se libRack.dylib non fosse risolvibile per @loader_path, dlopen fallirebbe qui
	// esattamente come fallirebbe dentro il DAW — è il gate sull'autosufficienza del bundle.
	void* dll = dlopen(modulePath.c_str(), RTLD_LOCAL | RTLD_NOW);
	if (!dll) {
		std::printf("FALLITO: dlopen del bundle: %s\n", dlerror());
		std::printf("  (\"image not found\" su libRack.dylib = il bundle NON è autosufficiente:\n");
		std::printf("   l'install name della dipendenza non è stato riscritto a @loader_path.)\n");
		return 1;
	}

	BundleEntryFn bundleEntryFn = (BundleEntryFn) dlsym(dll, "bundleEntry");
	BundleExitFn bundleExitFn = (BundleExitFn) dlsym(dll, "bundleExit");
	GetPluginFactoryFn getFactory = (GetPluginFactoryFn) dlsym(dll, "GetPluginFactory");
	CHECK(getFactory, "simbolo GetPluginFactory non trovato");
	CHECK(bundleEntryFn, "simbolo bundleEntry non trovato (l'host macOS lo richiede)");

	std::printf("bundleEntry()...\n");
	// L'host passa il CFBundleRef; l'adapter non lo usa (le risorse le trova da dladdr).
	CHECK(bundleEntryFn(nullptr), "bundleEntry ha fallito");
#endif
...
#else
	// Senza stub, l'adapter è il modulo che abbiamo caricato noi.
	MidiCountFn midiCount = (MidiCountFn) dlsym(dll, "MetarackDebugMidiCount");
#endif
	...
	v3_plugin_view** view = v3_cpp_obj(controller)->create_view(controller, "editor");
	CHECK(view, "create_view ha restituito NULL");
	CHECK(v3_cpp_obj(view)->is_platform_type_supported(view, V3_VIEW_PLATFORM_TYPE_NSVIEW) == V3_OK,
	      "la view rifiuta il tipo NSView");
	CHECK(v3_cpp_obj(view)->is_platform_type_supported(view, V3_VIEW_PLATFORM_TYPE_HWND) != V3_OK,
	      "la view accetta HWND su macOS");
	...
	// Finta finestra editor dell'host in cui incorporare il segnaposto, come farebbe un DAW.
	NSWindow* hostWnd = [[NSWindow alloc]
	                     initWithContentRect:NSMakeRect(0, 0, 400, 200)
	                     styleMask:NSWindowStyleMaskTitled
	                     backing:NSBackingStoreBuffered
	                     defer:NO];
	CHECK(hostWnd, "creazione della finta finestra host fallita");
	NSView* hostView = [hostWnd contentView];

	std::printf("attached() -> deve comparire il pannello accessibile MetaRack...\n");
	CHECK(v3_cpp_obj(view)->attached(view, hostView, V3_VIEW_PLATFORM_TYPE_NSVIEW) == V3_OK,
	      "attached fallita");
	CHECK([[hostView subviews] count] == 1,
	      "attached non ha aggiunto la view segnaposto dentro quella dell'host");

	std::printf("removed()...\n");
	CHECK(v3_cpp_obj(view)->removed(view) == V3_OK, "removed fallita");
	CHECK([[hostView subviews] count] == 0,
	      "removed non ha tolto la view segnaposto");
...
#if defined ARCH_WIN
#ifdef UNICODE
// -municode (compile.mk) seleziona l'entry wide.
